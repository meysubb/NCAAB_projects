...
agg.player <- read.csv(file="scraped_data/summary_player_data.tsv", sep="\t", header=TRUE, row.names=1, na.strings="?")
start.time <- Sys.time()
...
agg.player$minutes <- time_to_mins(agg.player,"minutes")
## Add new Assist Columns 
agg.player$astavg <- with(agg.player,ast/played)
agg.player$ast_to <- ifelse(agg.player$to==0,0, with(agg.player,ast/to))
...
agg.team <- read.csv(file="scraped_data/summary_team_data.tsv", sep="\t", header=TRUE, row.names=1, na.strings="?",stringsAsFactors = FALSE)
# Change Team to Character
agg.team$team_name <- as.character(agg.team$team_name)
# Convert Minutes from Character to Numeric
agg.team$team_minutes <- time_to_mins(agg.team,"team_minutes")
agg.team$opp_team_minutes <-  time_to_mins(agg.team,"opp_team_minutes")
# Remove All Commas
...
ind.game <- read.csv(file="scraped_data/game_data.tsv", sep="\t", header=TRUE, row.names=1, na.strings="?")
